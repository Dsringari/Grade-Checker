	...
	function _fetchEventSource(source, callback) {
		if (typeof source == 'string') {
			...
			$.ajax({
				...
				dataType: 'json',
				data: params,
				cache: options.cacheParam || false, // don't let $ prevent caching if cacheParam is being used
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg, className) {
		return "<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px'>" +
			"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + " onclick=\"showEvent('"+htmlEscape(event.Course)+"','"+strip(event.Description.replace(/\s+/g, ' '))+"','"+event.startdate+"','"+event.enddate+"','"+event.StudentEvent+"');\">" +
				"<span class='fc-event-bg'></span>" +
				"<span class='fc-event-time'>" + htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) + "</span>" +
				"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" +
			
			"</a>" +
				"<span class='fc-event-description' style='font-size:10px;'>"+htmlEscape(event.Description) +"</span>" +
			...
			html +=
				"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px'>" +
					"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + " onclick=\"showEvent('"+htmlEscape(event.Course)+"','"+strip(event.Description.replace(/\s+/g, ' '))+"','"+event.startdate+"','"+event.enddate+"','"+event.StudentEvent+"');\">" +
						...
						:'') +
						"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" +
						"<span class='fc-event-description'>"+event.Description +"</span>" +
					"</a>" +
